...
def bigphi_loss(model, config, inputs, outputs):
    ...
    return mutual_information_loss(outputs_logits, outputs)-mib_loss(evaluate_bipartition, config, outputs_logits, outputs)
...
def mib_loss(objective, config, X, Y):
    opcon=config["optuna"]
    assert X.shape[:1] == Y.shape[:1], f"First two dimension of X and Y must match: {(X.shape, Y.shape)}"
    d = X.shape[1]
    # Create a study with a pruner and optimize
    pruner = optuna.pruners.MedianPruner(**opcon["pruner"])
    study = optuna.create_study(direction='maximize', pruner=pruner)
    study.optimize(lambda trial: objective(trial, X, Y, mutual_information_loss, d), **opcon["optimizer"])
